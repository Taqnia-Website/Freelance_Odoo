...
class AgentRelation(models.Model):
    _name = 'agent.relation'
    _description = 'Agent Relation'
    _rec_name ='agent_id'

    '''def _get_shop_domain(self):
        company_ids = self.env.user.company_ids.ids
        # Define the domain to filter the sale shops
        domain = [('agent_id_company.company_ids', 'in', company_ids)]
        return domain'''


    agent_id = fields.Many2one('res.partner',string="Agent Name",domain=[('agent','=',True)])
    shop_ids = fields.Many2many('sale.shop', string="Hotel")
    

    
    @api.onchange('agent_id')
    def check_agent_onchnage(self):
        if self.agent_id:
            agent = self.agent_id.user_ids.company_ids
            for  agent_id in agent:
                shop_ids = self.env['sale.shop'].search([])
                for rec in shop_ids:
                    if rec.company_id == agent_id:
                        for shop in rec:
                            self.shop_ids = shop
    ...
    def check_agent(self):
        partner = self.search_count([('agent_id','=',self.agent_id.id)])
        if partner > 1:
            raise UserError("Agent Name is already exist.")
