...
from odoo import api, fields, models, tools, _, SUPERUSER_ID,exceptions
...
import calendar
from datetime import datetime, timedelta, timezone, time as tm
import logging
...
class hotel_reservation(models.Model):
    ...
    agent_res_id = fields.Many2one('res.users', 'Agent')
    agent_id_boolean = fields.Boolean('Split Agent ')
    agent_id_boolean_login = fields.Boolean('Split login ',compute="login_agent")
    agent_id_reservation = fields.Many2one('hotel.reservation', 'Main Reservation')

    #@api.onchange('agent_id_boolean')
    def login_agent(self):
        for rec in self:
            if self.env.user.has_group('hotel_management.group_agent'):
                rec.agent_id_boolean_login = True
            else:
                rec.agent_id_boolean_login = False


    @api.onchange('agent_id_boolean')
    def agent_id_boolean_onchange(self):
        for rec in self:
            for line in rec.reservation_line:
                if rec.agent_id_boolean == True:
                    line.agent_id_boolean_line = True
                else:
                    line.agent_id_boolean_line = False
    ...
    def split_reservation(self):
        new_reservations = self.env['hotel.reservation']

        for rec in self:
            today = datetime.today().date()
            tomorrow = today + timedelta(days=1)
        # Create a list of reservation lines to be added
            reservation_lines = []
            for line in rec.reservation_line:
                ...
                'checkin': today,
                'checkout': tomorrow,
                'agent_id_boolean_line':True,
                'price':line.room_number.lst_price,
                # Add any other fields you need to set for the new reservation line
            }))

        # Create a new reservation record with reservation lines
            new_reservation = self.env['hotel.reservation'].sudo().create({
            'partner_id': rec.partner_id.id,
            'deposit_policy': rec.deposit_policy,
            'shop_id': rec.shop_id.id,
            'date_order': rec.date_order,
            'agent_res_id': rec.env.user.id,
            'pricelist_id': rec.pricelist_id.id,
            'agent_id_reservation': rec.id,
            'agent_id_boolean': True,
            'reservation_line': reservation_lines,  # This will add the reservation lines
            # Add any other fields you need to set for the new reservation
        })

            #print("dddddddddddddddddddddddddddddd",new_reservation.reservation_line)

            new_reservations += new_reservation

    # Prepare the dictionary for redirection
            return {
                ...
                if housekeeping_room:
                    for house1 in housekeeping_room:
                        ...
                        house_current_date = (datetime.strptime(
                            str(house.current_date), '%Y-%m-%d %H:%M:%S')).date()
                        house_end_date = (datetime.strptime(
                            str(house.end_date), '%Y-%m-%d %H:%M:%S')).date()
        ...
        for res in self:
            if res:
                ...
                if res.reservation_line:
                    ...
                    #                           'individual room.')
        if 'state' in vals:
            hotel_update_template = self.env.ref('hotel_management.mail_template_reservation_update_hotel',
                                                 raise_if_not_found=False)
            company_update_template = self.env.ref('hotel_management.mail_template_reservation_update_company',
                                                   raise_if_not_found=False)
            client_update_template = self.env.ref('hotel_management.mail_template_reservation_update_client',
                                                  raise_if_not_found=False)
            client_confirm_template = self.env.ref('hotel_management.mail_template_reservation_confirm_client',
                                                  raise_if_not_found=False)
            if vals['state'] == 'confirm':
                company_partner_id = self.company_id.partner_id or self.env.company.partner_id
                client_confirm_template.send_mail(self.id, force_send=True, email_values={
                    'email_from': company_partner_id.email_formatted
                })
            for line in self.reservation_line:
                company_partner_id = self.company_id.partner_id or self.env.company.partner_id
                hotel_update_template.send_mail(
                    line.id, force_send=True,
                    email_values={
                        'email_from': company_partner_id.email_formatted
                    })
                client_update_template.send_mail(
                    line.id, force_send=True,
                    email_values={
                        'email_from': company_partner_id.email_formatted
                    })
                company_update_template.send_mail(
                    line.id, force_send=True,
                    email_values={
                        'email_from': company_partner_id.email_formatted, 'email_to': 'info@iroom.ai'
                    })
        return ret

    @api.model
    def _cron_send_feedback_mail(self):
        max_time = datetime.combine(fields.Date.today(), tm.max)
        min_time = datetime.combine(fields.Date.today(), tm.min)

        line_ids = self.env['hotel.reservation.line'].search([('checkout', '>=', min_time), ('checkout', '<=', max_time)])
        if line_ids:
            client_feedback_template = self.env.ref('hotel_management.mail_template_reservation_feedback_client',
                                                   raise_if_not_found=False)
            company_partner_id = self.company_id.partner_id or self.env.company.partner_id
            for reservation in line_ids.mapped('line_id'):
                client_feedback_template.send_mail(reservation.id, force_send=True, email_values={
                    'email_from': company_partner_id.email_formatted
                })
        ...
        if self.reservation_line:
            ...
            for line in self.reservation_line:
                if line.room_number:
                    ...
                    ctx.update({'date': line.checkin})
                    price = self.pricelist_id.with_context(ctx)._price_get(line.room_number, 1)[self.pricelist_id.id]

                    tax_ids = []
                    for tax_line in line.room_number.taxes_id:
    ...
    def _default_category_id(self):
        return [(6,
                 0,
                 [self.env.ref('hotel_management.res_partner_category_guest').id]
                )]
    ...
    via = fields.Selection([('direct', 'Direct'), ('agent', 'Agent')], "Via",
                           readonly=True, compute="compute_via",store=True)
    agent_id = fields.Many2one('res.partner', 'Agent', readonly=True,compute="compute_via",store=True)
    ...
    'account.payment',  # Model name for account payments
    'sale_order_account_payments_rel',  # Name of the table storing the relationship
    'sale_order_id',  # Field in the relationship table for sale orders
    'account_payment_id',  # Field in the relationship table for account payments
    "Payment Details"  # Optional label for the field in the user interface
    )
    user_partner_id =fields.Many2one('res.partner',compute='_current_user_partner_id')

    @api.depends('shop_id')
    def _current_user_partner_id(self):
        self.user_partner_id = self.env.user.partner_id.id
    ...
    def compute_via(self):
        if self.env.user.partner_id.agent:
            self.via = 'agent'
            self.agent_id =  self.env.user.partner_id.id
        else:
            self.via = 'direct'
                    ...
                    for history in room_line_id.room_folio_ids:
                        if history.state == 'done':
                            ...
                            if (history_start_date <= reservation_start_date < history_end_date) or (
                                    ...
                                    reservation_end_date >= history_end_date)):
                                if not (reservation.id == history.booking_id.id):
                                    if not self.agent_id_boolean == True:
                                        raise UserError("Room  %s is booked in this reservation period !" % (
                                        room_line_id.name))
                ...
                if housekeeping_room:
                    for house1 in housekeeping_room:
                        ...
                        if (house_current_date <= start_reser <= house_end_date) or (
                            ...
                            # if (((start_reser < house_current_date) and (end_reser > house_end_date)) or (house_current_date <= start_reser < house_end_date) or (house_current_date < end_reser <= house_end_date)) and (house.state == 'dirty'):
                            # print "i am in if loop--------------"
                            raise ValidationError("Room  %s is not clean for reservation period !" % (room_line_id.name))
    ...
    def create_folio(self):
        for reservation in self:
            room_his_id_search_agents = self.env['hotel.reservation'].search(
                [('agent_id_reservation', '=', reservation.id)])
            if room_his_id_search_agents:
            ...
            for line in reservation.reservation_line:
                ...
                if housekeeping_room:
                    for house1 in housekeeping_room:
                        ...
                        house_end_date = (
                            datetime.strptime(str(house.end_date), '%Y-%m-%d %H:%M:%S')).date()
                        start_reser = datetime.strptime(
                            str(line.checkin), '%Y-%m-%d %H:%M:%S').date()
                        end_reser = datetime.strptime(
                            str(line.checkout), '%Y-%m-%d %H:%M:%S').date()
            ...
            folio = self.env['hotel.folio'].create({
                ...
                'agent_id_res': reservation.agent_res_id.id,
                 'main_reservation_id': reservation.agent_id_reservation.id,
...
class hotel_reservation_line(models.Model):
    ...
    agent_id_boolean_line = fields.Boolean('Split Agent line ')
    agent_id_boolean_login_line = fields.Boolean('Split login ',compute="login_agent_line")

    #@api.onchange('agent_id_boolean')
    def login_agent_line(self):
        for rec in self:
            if self.env.user.has_group('hotel_management.group_agent'):
                rec.agent_id_boolean_login_line = True
            else:
                rec.agent_id_boolean_login_line = False

    def write(self, vals):
        # First check if we're changing dates or room that might cause an overlap
        if 'checkin' in vals or 'checkout' in vals or 'room_number' in vals:
            for record in self:
                ...
                if isinstance(checkin, str):
                    checkin = fields.Datetime.from_string(checkin)
                if isinstance(checkout, str):
                    checkout = fields.Datetime.from_string(checkout)

                room_number = vals.get('room_number', record.room_number.id)

                # Find other lines in the same reservation with the same room
                domain = [
                ...
                existing_lines = self.env['hotel.reservation.line'].search(domain)

                for existing in existing_lines:
                    # Check for overlap
                    if (checkin < existing.checkout and checkout > existing.checkin):
                        raise UserError(
                            f'Cannot have overlapping dates for the same room. Room is already reserved from {existing.checkin} to {existing.checkout}.')
    ...
    def create(self, vals_list):
        for val in vals_list:
            ...
            # Check for overlap within the same reservation
            if 'line_id' in val and val['line_id'] and 'room_number' in val and val['room_number']:
                existing_lines = self.env['hotel.reservation.line'].search([
                    ('line_id', '=', val['line_id']),
                ...
                for existing in existing_lines:
                    ...
                    if (checkin < existing.checkout and checkout > existing.checkin):
                        raise UserError(
                            f'Cannot add the same room twice with overlapping dates. Room is already reserved from {existing.checkin} to {existing.checkout}.')

        so = super(hotel_reservation_line, self).create(vals_list)
        return so
    ...
    def onchange_date_count_total_days(self):
        for rec in self:
            # Only run the validation if this is an agent-split reservation
            if rec.line_id and rec.line_id.agent_id_reservation:
                booking_main = self.env['hotel.reservation'].sudo().search(
                    [('id', '=', rec.line_id.agent_id_reservation.id)])
                for book_line_main in booking_main:
                    for res_line_main in book_line_main.reservation_line:
                        if res_line_main.checkin > rec.checkin:
                            raise ValidationError("Please select the checkin date between booked reservation")
    ...
    # @api.depends('line_id.shop_id')
    def onchange_date_count_total_dayss(self):
        for rec in self:
            # Only run the validation if this is an agent-split reservation
            if rec.line_id and rec.line_id.agent_id_reservation:
                booking = self.env['hotel.reservation'].sudo().search(
                    [('agent_id_reservation', '=', rec.line_id.agent_id_reservation.id)])
                for book_line in booking:
                    for res_line in book_line.reservation_line:
                        if res_line.checkout == rec.checkout:
                            raise ValidationError("Please select the checkin date between booked reservation")
                ...
                for book_line_main in booking_main:
                    for res_line_main in book_line_main.reservation_line:
                        if res_line_main.checkout < rec.checkout:
                            raise ValidationError("Please select the checkout date between booked reservation")
    ...
    @api.depends('checkin', 'checkout', 'banquet_id.pricelist_id', 'banquet_id', 'line_id.partner_id', 'number_of_days',
                 'banquet_id.source')
    def onchange_room_id_test(self):
        v = {}
        res_list = []
        warning = ''
                ...
                for history in room_line_id.room_folio_ids:
                    if history.state == 'done':
                        ...
                        if housekeeping_room:
                            for house1 in housekeeping_room:
                                house = house1
                                house_current_date = (datetime.strptime(str(house.current_date), '%Y-%m-%d %H:%M:%S')).date()
    ...
    def count_sub_total1(self):
        for line in self:
            ...
            taxes = line.sudo().taxes_id.compute_all(price, line.line_id.currency_id, line.number_of_days,
    ...
    def default_get(self, default_fields):
        ...
        if self.env.user.id == 4 or self.env.user._is_public:
    ...
    def _get_display_price(self, product):
        ...
        if self.line_id.pricelist_id.discount_policy == 'with_discount':
            ...
            return product.with_context(pricelist=self.line_id.pricelist_id.id).list_price
...
class hotel_folio(models.Model):
    ...
    def update_folio_line_state(self):
        folios = self.env['hotel.folio'].sudo().search([('state', '=', 'check_out')])
        for folio in folios:
            for line in folio.room_lines:
                line.folio_state = folio.state
    ...
    payment_move_ids = fields.Many2many(
    'account.payment',  # Model name for account payments
    'sale_order_account_payment_rel',  # Name of the table storing the relationship
    'sale_order_id',  # Field in the relationship table for sale orders
    'account_payment_id',  # Field in the relationship table for account payments
    "Payment Details"  # Optional label for the field in the user interface
    )
    ...
    main_reservation_id = fields.Many2one('hotel.reservation', 'Main Reservation Ref')
    agent_id_res = fields.Many2one('res.users', 'Spilt Agent')
    ...
    def action_checkout(self):
        ...
        for room in folio.room_lines:
            ...
            housekeeping_id = self.env['hotel.housekeeping'].create({
                'current_date': sdate,
                'end_date': (sdate + timedelta(hours=2)),
                'clean_type': 'checkout',
                'room_no': room.product_id.id,
                'inspector': self._uid,
                'inspect_date_time': sdate + timedelta(hours=1),
                'state': 'dirty',
                'quality': 'clean',
            })
...
class hotel_restaurant_order(models.Model):
    ...
    def onchange_folio_id(self):
        ...
        if not self.folio_id:
            ...
            if len(folio.room_lines) < 2:
                room = self.env['hotel.room'].search([('product_id','=',folio.room_lines.product_id.id)])
                res['room_no'] = room.id
                res['partner_id'] = folio.partner_id.id
                res['shop_id'] = folio.reservation_id.shop_id.id
            else:
                res['partner_id'] = folio.partner_id.id
                res['shop_id'] = folio.reservation_id.shop_id.id
            return {'value':res}
