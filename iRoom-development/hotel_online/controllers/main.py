...
class HotelRoomDashboardController(http.Controller):
    ...
    def your_method(self):
        ...
        # Redirect or return the result as per your requirements
...
class PaymentPortal(payment_portal.PaymentPortal):
    ...
    def shop_payment_transaction(self, order_id, access_token, **kwargs):
        ...
        if request.session.get('reservation_order_id'):
            if not kwargs.get('amount', False):  # Express
                kwargs['amount'] = order_sudo.total_cost1
            if tools.float_compare(kwargs['amount'], order_sudo.total_cost1, precision_rounding=order_sudo.currency_id.rounding):
                #raise ValidationError(_("The cart has been updated.1 Please refresh the page."))
                print("g")
            if tools.float_compare(kwargs['amount'], order_sudo.remaining_amt, precision_rounding=order_sudo.currency_id.rounding):
                raise ValidationError("The Reservation has no unpaid amount.")
        elif request.session.get('sale_order_id'):
            if not kwargs.get('amount', False):  # Express
        ...
        if request.session.get('reservation_order_id'):
            ...
            if order.remaining_amt == 0:  # Prevent creating transaction if no remaining mount
                raise ValidationError("The Reservation has already paid.")
            tx_sudo = self._create_transaction(
                custom_create_values={'reservation_ids': [Command.set([order_id])]}, **kwargs,
            )
        elif request.session.get('sale_order_id'):
            kwargs.update({
                'reference_prefix': None,  # Allow the reference to be computed based on the order
...
class check(http.Controller):
    ...
    def search_products(self, **kwargs):
        ...
        shop_id = int(kwargs.get('hotel_select')) if kwargs.get('hotel_select') else ''

        today = fields.Date.today()
        if 'main-input-search_slider' in kwargs:
            if not kwargs.get('main-input-search_slider'):
                raise UserError("Please Enter Date Range")

            dates = kwargs.get('main-input-search_slider').split(' - ')
            if len(dates) != 2:
                raise UserError("Please Enter Correct Date Range")

            start_date = datetime.strptime(dates[0], '%m/%d/%Y').date()
            end_date = datetime.strptime(dates[1], '%m/%d/%Y').date()
            if start_date < today or end_date < today:
                raise UserError("Please Enter Date range after today's date")
        else:
            ...
            if start_date < today or end_date < today:
                raise UserError("Please Enter Checkin Date And Checkout Date after today's date")

        self._uid = SUPERUSER_ID
        product_id = request.env['product.product'].sudo().search([]).ids
        check_room_id = request.env['hotel.room_type'].sudo().search([])
        amenities_idsss = request.env['hotel.room_type'].sudo().search([]).ids
        services_idsss = request.env['hotel.services'].sudo().search([]).ids
        for i in check_room_id:
            cat_id = i.cat_id.id
            room = request.env['hotel.room'].sudo().search([('categ_id','=',cat_id),('shop_id','=',shop_id)])
            if room:
                room_idsss.append(i.id)
        ...
        if room_idsss:
            ...
            for rbrw in room_brw:
                ...
                if room_ids111:
                    ...
                    shop_brw = request.env['sale.shop'].sudo().browse(shop_id)
    ...
    def reserv_products(self, **kwargs):
        ...
        room_id = ''
        self._uid = SUPERUSER_ID
        shop_id = int(kwargs.get('shop_id')) if kwargs.get('shop_id') else ''
        user = request.env['res.users'].sudo().browse(request.uid)
        company = request.env.user.sudo().company_id
        ...
        # print(names)
        if not names:
            raise UserError("The name can not be empty, please set the name")
        ...
        #print(kwargs['len'])
        try:
            no = ast.literal_eval(kwargs['len'])
        except Exception as e:
            no = (kwargs['len'])

        if not any([key.startswith('sel_') for key in kwargs.keys()]):
            raise ValidationError("You must select a room type before you can proceed")
        total_no_adults = 0
        total_no_childs = 0
        for l1 in no:
            no_of_childs = 0
            ...
            if not str4 in kwargs:
                continue
            if str1 in kwargs:
                record.update({'room_type': int(kwargs[str1])})
                record.update({'no_room': 1})  # To be replaced with correct value

            if str2 in kwargs:
                if kwargs[str2].isnumeric():
                    record.update({'child': int(kwargs[str2])})
                    no_of_childs = int(kwargs[str2])
                else:
                    record.update({'child': 0})
                    no_of_childs = 0

            if str3 in kwargs:
                if kwargs[str3].isnumeric():
                    if int(kwargs[str3]) <= 0:
                        raise UserError("Adults number should be higher than 0")
                    record.update({'adult': int(kwargs[str3])})
                    no_of_adults += int(kwargs[str3])
                else:
                    raise UserError("Adults number should be a valid number")

            if str4 in kwargs:
                record.update({'chk': kwargs[str4]})

            if str5 in kwargs:
                if kwargs[str5].isnumeric():
                    if int(kwargs[str3]) <= 0:
                        raise UserError("Rooms number should be higher than 0")
                    ...
                    raise UserError("Rooms number should be a valid number")
        ...
        if not room_id:
            ...
            _logger.info("...........................vals8====>>>>>>>>>%s", vals)
            room_id = request.env['hotel.reservation'].sudo().create(vals)
            _logger.info("RESERVATION ID2678====>>>>>>>>>%s", room_id)
        ...
        for rtype in rm_types:
            ...
            # if 'chk' in rtype:
                if rtype['chk'] == 'on':
                    for lno in range(0, (int(rtype['no_room']))):
                        no_of_days = (datetime.strptime(newdate1, '%Y-%m-%d %H:%M:%S') - datetime.strptime(
                            newdate, '%Y-%m-%d %H:%M:%S')).total_seconds()
                        ...
                        if len(res1) > 0:
                            ...
                            room_line_id = request.env['hotel.reservation.line'].sudo().create(vals)
                            _logger.info("\n\n\n\n\n\n ROOM ID===>>>>>>%s",room_line_id.line_id)

                            if res1[lno].taxes_id.id:
                                room_line_id.update({'taxes_id': [(6, 0, [res1[lno].taxes_id.id])]})
                            room_line_id.sudo().line_id.compute()
            ...
            # if 'chk' in rtype:
                ...
                for lll in range(1, (int(rtype['no_room'] + 1))):
                    dict = {'rm_name': room_brw.name, 'room_type_id': rtype['room_type'], 'no_room': rtype['no_room']}
                    if room_brw.img_ids:
        ...
        if request.httprequest.method == 'GET':
            ...
            for room in room_data:
                key = room['room_type_id']  # or d['name'], or (d['id'], d['name']), etc.
                if key not in room_type_ids:
                    room_type_ids.add(key)
                    unique_room_data.append(room)
            if unique_room_data:
                room_data = unique_room_data
    ...
    def checkout_form_save(self, checkout):
        ...
        if not partner_id:
            if request.uid != request.website.user_id.id:
                ...
                user_ids = request.env['res.users'].sudo().search([("partner_id", "=", order.partner_id.id)])
                if not user_ids or request.website.user_id.id not in user_ids:
                    partner_id = order.partner_id.id
    ...
    def checkout(self, **post):
        ...
        if request.session.get('reservation_order_id'):
            ...
            reservation_sudo = request.env['hotel.reservation'].sudo().browse(reservation)
            if not reservation_sudo.reservation_line:
                raise ValidationError("You can not proceed to payment before choosing at least one room")
            values = self.checkout_values()
    ...
    def confirm_order(self, **post):
        ...
        user_id = request.env['res.users'].sudo().search([('id', '=', request.uid)])
        if user_id:
            partner_id = order.partner_id if order else user_id.partner_id
            if partner_id:
    ...
    def payment(self, **post):
        ...
        if request.session.get('reservation_order_id'):
            ...
            if order_browse.remaining_amt == 0:
    ...
    def shop_payment_validate(self, transaction_id=None, sale_order_id=None, **post):
        ...
        #  TODO: no remove_transactions() fn found
    ...
    def other_items_unlink(self, **post):
        ...
        other_items_id = request.env['other.items'].sudo().search(
            [('id', '=', int(post.get('other_items_id', 0)))])
    ...
    def get_products(self):
        ...
        if user.partner_id.agent:
            ...
            #print("22222222222222222222222",shop.shop_ids)
            for i in shop:
                #print("66666666666666666666666666666",i.shop_ids)
                for s in i.shop_ids:
                    #print("333333333333333333333333333333",i)
                    res.append({
                        'name': s.name,
                        'id':s.id
                    })  
