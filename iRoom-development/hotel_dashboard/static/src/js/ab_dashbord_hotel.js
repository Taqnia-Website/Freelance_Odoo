  ...
  async reload() {
    ...
    if (shop_id) {
      self.next_date.setDate(self.next_date.getDate() - cnt);
        const { room_types, folio, cleaning, repair } = await this.orm.call('hotel.reservation', 'get_all_reservation_details', [0], { shop_id });
       let roomtype = room_types;
      for (var i = 0; i < roomtype.length; i++) {
        ...
        var room = roomtype[i].rooms
        for (var j = 0; j < room.length; j++) {
          var tss = "";
          var room_id = room[j].id;
          var cater_id = roomtype[i].id;
          var shop_id = $("#shops").val();
          var hotelroom = room[j].reserve_room;
          var dates = [];
          for (var h = 0; h < hotelroom.length; h++) {
            ...
            var checkout = hotelroom[h].checkout;
      ...
      var folio_detail = folio;

      for (let f = 0; f < folio_detail.length; f++) {
              const folio = folio_detail[f];
              let checkin = folio.checkin.split(" ")[0];
              let checkout = folio.checkout.split(" ")[0];
              let status = folio.status;

              // Normalize room name by replacing spaces with hyphens
              let froomname = folio.room_name.includes(" ") ? folio.room_name.replace(/ /g, "-") : folio.room_name;

              let checkin_date = new Date(checkin);
              let checkout_date = new Date(checkout);

              // Set the check-in cell text
              const ckn = $(`#${froomname}_${checkin}`);
              ckn.text(folio.fol_no);

              // Set background color based on status
              let backgroundColor = status === "check_out" ? "#fcd0bb" : "#b5dcd1";
              ...
              while (checkin_date <= checkout_date) {
                    ...
                    chin.attr("data-id", folio.id);
                    chin.css("background-color", backgroundColor);
                    chin.css("disply", "flex");
                    chin.css("max-width", "40px");
                    chin.css("overflow", "unset");
                    checkin_date.setDate(checkin_date.getDate() + 1);
