...
class hotel_reservation(models.Model):
    ...
    pick_up = fields.Selection([('yes', 'Yes'), ('no', 'No')], 'Is Pickup Required', readonly=True, default=lambda *a: 'no')
    chargeable = fields.Boolean('Is Chargeable', readonly=True)
    service_type = fields.Selection([('internal', 'Internal'), ('third_party', 'Third Party')], 'Service Type',
                                    readonly=True, default=lambda *a: 'internal')
    trans_partner_id = fields.Many2one('transport.partner', 'Transporter Name', readonly=True)
    pickup_time = fields.Datetime('Pickup Time', readonly=True)
    source_id = fields.Many2one('location.master', 'Pickup Location', readonly=True)
    destination_id = fields.Many2one('location.master', 'Destination', readonly=True)
    trans_mode_id = fields.Many2one('transport.mode', 'Transport Mode', readonly=True)
    ...
    def cancel_reservation(self):
        ...
        return super(hotel_reservation, self).cancel_reservation()
...
class TransportTask(models.Model):
    ...
    def get_project_id(self):
        ...
        if not obj:
            raise UserError(
                "Warning! There is no project name as Transportation !")
        return obj[0]
    ...
    def action_close(self):
        ...
        for obj in self:
            if obj.reservation_id:
                ...
                if not price_ids:
                    raise UserError(
                        _("Warning! Wrong location for Pickup and Destination."))
                price_id = price_ids
                ...
                if obj.service_type != 'internal' and obj.is_chargeable and obj.is_pickup:
                    ...
                    # invoice_data_id1._compute_name()
                    # invoice_data_id1._compute_currency_id()
                    ...
                    # # invoice_line_id._compute_account_id()
                    # # invoice_line_id._compute_name()
                    # # invoice_line_id._compute_tax_ids()
                    # # invoice_line_id._compute_product_uom_id()
                    # # invoice_line_id._compute_price_unit()
                    # # invoice_line_id._compute_debit_credit()
                    # # invoice_line_id._inverse_credit()
                    # # invoice_line_id._onchange_product_id()
                    # # invoice_line_id.price_unit = cost_price
                    # # invoice_line_id._onchange_mark_recompute_taxes()
                    # # invoice_line_id._compute_totals()
                    # # invoice_line_id._get_fields_onchange_balance()
                    # # invoice_line_id._compute_balance()
                    # # invoice_data_id1._compute_tax_totals()
                    ...
                    invoices = self.env['account.move'].create([
                    {
                        ...
                        'currency_id': obj.trans_partner_id.partner_id.property_product_pricelist.currency_id.id,
                        'journal_id': len(pur_journal_ids) and pur_journal_ids[0].id or False,
                        'invoice_line_ids': [(0, 0, {'name': product.name,'product_id': product.id,'price_unit': cost_price,'quantity': 1,'product_uom_id': product.uom_id.id,'currency_id': obj.trans_partner_id.partner_id.property_product_pricelist.currency_id.id,'tax_ids': [(6, 0, tax_ids)]})],
                    }])
    ...
    def _message_get_suggested_recipients(self):
        task_id = self.task_id
        res = super(TransportTask, self)._message_get_suggested_recipients()
